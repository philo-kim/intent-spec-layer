...
export const method = "full-spec-layer";
export const notes = [
...
export function createSystem() {
  ...
  function validateCoupon(order, coupon) {
    ...
    if (rawTotal(order) < coupon.min_amount) {
      throw new SpecError("MinimumOrderAmount", "Order does not meet minimum order amount.");
    }
  }

  return {
    terms: ["account_id", "order_id", "coupon_id", "line_item_id", "total_amount"],
    ...
    confirmOrder(orderId, options = {}) {
      ...
      const idempotencyId = idempotencyKey ? `${orderId}:${idempotencyKey}` : null;
      if (idempotencyId && idempotency.has(idempotencyId)) {
        return idempotency.get(idempotencyId);
      }
      if (order.status === "cancelled") {
        throw new SpecError("InvalidTransition", "Cancelled order cannot be confirmed.");
      }
      const runtimeInventory = buildInventory(options.inventoryPlan);
      const reservedItems = [];
      try {
        for (const item of order.items) {
          runtimeInventory.reserve(item.line_item_id);
          inventory.stats.reservations.push(item.line_item_id);
          reservedItems.push(item.line_item_id);
        }
        if (options.paymentSucceeds === false) {
          throw new SpecError("PaymentFailed", "Payment failed.");
        }
        runtimeInventory.charge(orderId);
        inventory.stats.charges.push(orderId);
        order.status = "confirmed";
        if (order.coupon) {
          order.coupon.used = true;
        }
        const result = { ...order, total_amount: total(order) };
        if (idempotencyId) {
          idempotency.set(idempotencyId, result);
        }
        return result;
      } catch (error) {
        for (const lineItemId of reservedItems.toReversed()) {
          runtimeInventory.release(lineItemId);
          inventory.stats.releases.push(lineItemId);
        }
        throw error;
      }
    },
